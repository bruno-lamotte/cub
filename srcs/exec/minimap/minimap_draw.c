...
static void	sub_render_cell(t_engine *eng, t_vec2 pos, t_vec2 cell, double g[2])
{
	int				idx;
	t_vec2			d;
	int				d2;
	...
	scale = get_minimap_scale(eng, 90);
	d = (t_vec2){.i = {pos.i.x - (eng->screen->win_width - 120),
		pos.i.y - 120}};
...
static void	draw_minimap_cell(t_engine *eng, t_vec2 pos, t_vec2 c, double scale)
{
	...
	sub_render_cell(eng, pos, cell, g);
}
...
void	draw_minimap(t_engine *eng)
{
	...
	while (++pos.i.y <= c.i.y + radius)
	{
		...
		{
			if (pos.i.x >= 0 && pos.i.x < eng->screen->win_width
				&& pos.i.y >= 0 && pos.i.y < eng->screen->win_height)
				draw_minimap_cell(eng, pos, c, scale);
		}
	}
	m_scale.d.x = radius;
	m_scale.d.y = 1.0 / scale;
	draw_minimap_monsters(eng, c, m_scale);
