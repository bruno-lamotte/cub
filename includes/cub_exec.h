...
void				draw_minimap_monsters(t_engine *eng, t_vec2 c,
						t_vec2 scale);
...
void				init_row_params(t_worker *w, int y, t_floor_row *r,
						t_lut *lut);
void				draw_floor_pixel(t_worker *w, t_floor_row *r, int x,
						float sh[2]);
...
int					check_player_mstr_collision(double x, double y,
						t_engine *eng);
...
int					check_thick_los(t_vec2 p1, t_vec2 p2, double radius,
						void *blob);
int					check_los_shadow(t_vec2 m, t_vec2 p);
int					run_dda(int step[4], float d[4], float side[2],
						t_map_data *map);
float				compute_light_at_point(double wx, double wy, void *blob,
						t_engine *eng);
...
void				update_single_monster(t_monster_rt *m, t_engine *eng,
						t_worker *w);
...
t_vec2				find_next_step(t_monster_rt *m, t_engine *eng,
						t_vec2 target, t_worker *w);
void				mstr_move_towards(t_monster_rt *m, t_engine *eng,
						t_vec2 target);
void				mstr_go_to_guard(t_monster_rt *m, t_engine *eng,
						t_worker *w);
void				mstr_chase(t_monster_rt *m, t_engine *eng, t_worker *w);
int					get_fsm_state_from_slr_state(t_engine *eng,
						int slr_state_id);
...
int					score_patrol_cell(t_engine *eng, t_monster_rt *m,
						t_vec2 cell);
...
int					bfs_run(t_vec2 start, t_vec2 target, t_engine *eng,
						t_worker *w);
...
int					collect_monsters(t_engine *eng, t_sprite *sprites,
						int count);
int					collect_objects(t_engine *eng, t_sprite *sprites,
						int count);
...
void				parse_dxpm_row(char *line, t_img *dst,
						unsigned int *palette, int cpp);
